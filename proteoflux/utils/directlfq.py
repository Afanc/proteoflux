# from https://github.com/MannLabs/directlfq
...
def get_list_with_multiprocessing(input_specification_tuplelist_idx__df__num_samples_quadratic__min_nonan, num_cores):
    ...
    it = input_specification_tuplelist_idx__df__num_samples_quadratic__min_nonan
    try:
        total = len(it)
    except TypeError:
        it = list(it); total = len(it)  # only to show a correct total
    wrapped = tqdm(it, total=total, desc="DirectLFQ", file=sys.stdout, leave=False)
    list_of_tuple_w_protein_profiles_and_shifted_peptides = pool.starmap(
        calculate_peptide_and_protein_intensities, wrapped
    )
    pool.close()
    return list_of_tuple_w_protein_profiles_and_shifted_peptides
...
def get_configured_multiprocessing_pool(num_cores):
    ...
    #log_info(f"using {pool._processes} processes")
    return pool
