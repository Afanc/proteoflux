#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Author: Boris Muzellec, https://raw.githubusercontent.com/BorisMuzellec/MissingDataOT/master/utils.py
...
def produce_balanced_mv(X, p_miss_total=0.1, p_mnar=0.5, quantile=0.2, q_lod=None, **kwargs):
    """
    Generate balanced MAR and MNAR missing values, optionally adding a fixed LOD censoring step.

    Parameters
    ----------
    X : torch.DoubleTensor or np.ndarray, shape (n, d)
        Data matrix where rows are samples and columns are variables.
        Converted to torch.tensor if input is numpy array.

    p_miss_total : float, default 0.1
        Total fraction of missingness to simulate.

    p_mnar : float, default 0.5
        Fraction of missingness that is MNAR (remainder MAR).

    quantile : float, default 0.2
        Quantile level for MNAR quantile-based censoring.

    q_lod : float, optional, default None
        Quantile to apply strict limit-of-detection (LOD) censoring.
        If provided, all values below this quantile per variable are systematically censored.

    Returns
    -------
    dict
        Dictionary containing:
            - 'X_init': Original input matrix (torch tensor).
            - 'X_incomp': Data matrix after simulated missingness (torch tensor).
            - 'mask': Boolean mask indicating positions of generated missing values (torch tensor).
    """
    ...
    # Compute MAR and MNAR proportions
    p_miss_mnar = p_miss_total * p_mnar
    p_miss_mar = p_miss_total - p_miss_mnar

    # Generate MAR missingness
    mar_result = produce_NA(X, p_miss_mar, mecha="MAR", p_obs=0.3)

    # Generate MNAR missingness (quantile-based)
    mnar_result = produce_NA(X, p_miss_mnar, mecha="MNAR", opt="quantile", p_obs=0.3, q=quantile)

    # Combine MAR and MNAR masks
    combined_mask = torch.logical_or(mar_result['mask'].bool(), mnar_result['mask'].bool())

    # Add explicit LOD censoring if specified
    if q_lod is not None:
        lod_thresholds = torch.quantile(X, q=q_lod, dim=0, keepdim=True)
        lod_mask = X <= lod_thresholds
        combined_mask = torch.logical_or(combined_mask, lod_mask)

    # Apply combined mask to data
    X_nas = X.clone()
    X_nas[combined_mask] = torch.nan

    # ensure at least one observed value per row
    # find rows that became entirely NaN
    row_all_nan = torch.isnan(X_nas).all(dim=1)
    if row_all_nan.any():
        # for each fully‐NaN row, randomly pick one column to keep
        nan_row_idxs = row_all_nan.nonzero(as_tuple=False).view(-1)
        for i in nan_row_idxs.tolist():
            # pick a random sample column
            j = torch.randint(0, X_nas.size(1), (1,)).item()
            # restore the original value at (i,j)
            X_nas[i, j] = X[i, j]
            # update the mask to reflect that it's now observed
            combined_mask[i, j] = False
